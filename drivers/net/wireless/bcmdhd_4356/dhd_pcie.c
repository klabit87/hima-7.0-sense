...
dhdpcie_bus_suspend(struct dhd_bus *bus, bool state)
{
	...
	if (state) {
		...
		/* check bus error/islinkdown status after dhd_os_set_ioctl_resp_timeout */
		if (dhd_query_bus_erros(bus->dhd)) {
			return BCME_ERROR;
		}
		...
		if (bus->wait_for_d3_ack) {
			...
			if (!bus->force_suspend && dhd_os_check_wakelock_all(bus->dhd)) {
				...
				/* check bus error before access si_corereg */
				if (dhd_query_bus_erros(bus->dhd)) {
					return BCME_ERROR;
				}
			...
			DHD_ERROR(("%s: d3ack_timeout_occured Set TRUE\n", __func__));
